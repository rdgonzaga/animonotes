            ...
            {!showSignIn ? (
              <>
                ...
                <p className="text-white/70 text-sm">
                  ...
                  <button onClick={() => setShowSignIn(true)} className="text-white font-medium underline">
                    Sign in
                  </button>
                </p>
              </>
              ...
              <form onSubmit={handleSubmit} className="space-y-3 text-left">
                ...
                {error && (
                  <div className="text-sm text-white bg-red-500/30 p-2 rounded-lg text-center">{error}</div>
                )}
                <Button type="submit" className="w-full bg-white text-foreground hover:bg-white/90 rounded-lg" disabled={loading}>
                  {loading ? "Signing in..." : "Sign In"}
                </Button>
                <p className="text-white/70 text-sm text-center">
                  <button type="button" onClick={() => setShowSignIn(false)} className="text-white font-medium underline">
                    Back to sign up
                  </button>
                  {" · "}
      ...
      <section className="bg-background py-16 md:py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="font-serif text-2xl md:text-4xl text-primary mb-12">
            Stop scrambling and start <span className="underline decoration-primary/30 decoration-2 underline-offset-4">sharing</span>
          </h2>
